...
class FATTLS:
    ...
    def fatt(self, signal):
        ...
        # Querries are q = signal.T, so the Q * K^T * n^a part is:
        t = np.matmul(signal.T, self.keys.T) * self.norm
        t = np.sqrt(
            t[0, 0:self.f_matrix.shape[0]] ** 2 +
            t[0, self.f_matrix.shape[0]:] ** 2
        )
        # The attention matrix is s = softmax(t - max(t)), so the output is:
