...
class QIFFT:
    ...
    def reconstruct(self, freqs, amps, phases):
        """Reconstruct signal based on the estimated QIFFT sinusoidal parameters."""
        return np.sum(
            amps[:, np.newaxis] * np.cos(self.pi2t * freqs[:, np.newaxis] + phases[:, np.newaxis]), axis=0
        )

    def fft(self, signal):
        """Return half the fft spectrum (signals are real) and the axis."""
    ...
    def estimate(self, signal):
        """Estimate the sinusoidal parameters of the input signal with the Quadratically Interpolated Fast Fourier
        Transform (QIFFT) method."""
        # Apply window:
        sig = signal * self.w
        ...
        # Initialize:
        freqs, amps, phases = np.zeros(len(peaks)), np.zeros(len(peaks)), np.zeros(len(peaks))
        # Main loop:
        for i, peak in enumerate(peaks):

            # Estimate each sinusoid frequency and amplitude based on quadratic interpolation:
            x1, x2, x3 = peak - 1, peak, peak + 1
            y1, y2, y3 = sig_abs_fft[x1], sig_abs_fft[x2], sig_abs_fft[x3]
            x0 = ((y3 - y2) * (x2 + x1) - (y2 - y1) * (x3 + x2)) / (2 * (y3 - 2 * y2 + y1))
            a = (y2 - y1) / (x2 + x1 - 2 * x0)
            y0 = y1 - a * (x1 - x0) ** 2
        ...
        # Sort the [amplitude,frequency,phase] triplets based on amplitude in descending order:
        amps, freqs, phases = zip(*sorted(zip(amps, freqs, phases), reverse=True))
        amps, freqs, phases = np.array(amps), np.array(freqs), np.array(phases)

        if self.no_sins < len(freqs):
            freqs, amps, phases = freqs[0:self.no_sins], amps[0:self.no_sins], phases[0:self.no_sins]
